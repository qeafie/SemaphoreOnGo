...
1. **Задачи для самостоятельного выполнения**  
...
- **Надо:** Структура `Semaphore` и её методы уже реализованы.  
  **Решение:** См. файл `semaphore.go`

- **Надо:** Пример запуска нескольких горутин с семафором.  
  **Решение:** См. файл `main.go`

- **Надо:** Обработка результатов через канал с возможностью преждевременной остановки при ошибке.  
  **Решение:** См. файл `semaphore.go`
...
Ниже перечислены русскоязычные статьи, в которых подробно описываются семафоры, горутины и каналы в Go. Рекомендуется ознакомиться с ними для теоретического обоснования и получения дополнительных примеров.
...
└── …              # Дополнительные файлы и пакеты (при необходимости)
